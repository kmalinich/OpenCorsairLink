...
corsairlink_coolit_fan_mode_read_rpm(struct corsair_device_info* dev, struct libusb_device_handle* handle, struct fan_control* ctrl) {
	...
	rr = dev->driver->write(handle, dev->write_endpoint, commands, 64);
	rr = dev->driver->read(handle, dev->read_endpoint, response, 64);
	ctrl->rpm_current = (response[3] << 8) + response[2];
...
corsairlink_coolit_fan_mode_rpm(struct corsair_device_info* dev, struct libusb_device_handle* handle, struct fan_control* ctrl) {
	...
	commands[++ii] = FAN_FixedRPM;
	commands[++ii] = ctrl->rpm_current & 0xFF;
	commands[++ii] = (ctrl->rpm_current >> 8) & 0xFF;
...
corsairlink_coolit_fan_speed(struct corsair_device_info* dev, struct libusb_device_handle* handle, struct fan_control* ctrl) {
	...
	ctrl->rpm_current = (response[5] << 8) + response[4];
	ctrl->rpm_max     = (response[9] << 8) + response[8];
