...
int
__wrap_corsairlink_asetek_write(struct libusb_device_handle* dev_handle, uint8_t endpoint, uint8_t* data, int length) {
...
int
__wrap_corsairlink_asetek_read(struct libusb_device_handle* dev_handle, uint8_t endpoint, uint8_t* data, int length) {
	check_expected(endpoint);
	check_expected(data);
	check_expected(length);
	return mock();
...
test_corsairlink_asetek_firmware_id_failure(void** state) {
	...
	expect_value(__wrap_corsairlink_asetek_write, endpoint, 0x02) expect_value(__wrap_corsairlink_asetek_write, data, commands);
	expect_value(__wrap_corsairlink_asetek_write, length, 32);
	will_return(__wrap_corsairlink_asetek_write, -1);

	expect_value(__wrap_corsairlink_asetek_read, endpoint, 0x82) expect_value(__wrap_corsairlink_asetek_read, data, response);
