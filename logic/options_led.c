...
led_suboptions_parse(char* subopts, struct led_control* settings) {
	...
	while (*subopts != '\0') switch (getsubopt(&subopts, led_options, &value)) {
		...
		case SUBOPTION_LED_COLORS: /* led color */
			...
			while (token != NULL) {
				...
				sscanf(token, "%02hhX%02hhX%02hhX,", &settings->led_colors[ii].red, &settings->led_colors[ii].green, &settings->led_colors[ii].blue);
				++ii;
				token = strtok(NULL, ":");
			}
			break;
		...
		case SUBOPTION_LED_WARN: /* led warning color */
			sscanf(value, "%02hhX%02hhX%02hhX", &settings->led_colors[2].red, &settings->led_colors[2].green, &settings->led_colors[2].blue);
			break;
