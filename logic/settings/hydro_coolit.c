...
int
hydro_coolit_settings(struct corsair_device_scan scanned_device, struct option_flags flags, struct option_parse_return settings) {
	int rr;
	int ii;
	char name[20];
	...
	for (ii = 0; ii < readings.fan_ctrl.fan_count; ii++) {
		...
		msg_info("\tCurrent/Max Speed %i/%i RPM\n", readings.fan_ctrl.rpm_current, readings.fan_ctrl.rpm_max);
	}
