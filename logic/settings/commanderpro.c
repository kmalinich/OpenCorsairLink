...
int
commanderpro_settings(struct corsair_device_scan scanned_device, struct option_flags flags, struct option_parse_return settings) {
	int rr;
	int ii;
	char name[32];
	...
	/* fetch SATA voltages */
	for (ii = 0; ii < 3; ii++) {
		if (ii == 0) msg_info("Output 12v: ");
		if (ii == 1) msg_info("Output 5v: ");
		if (ii == 2) msg_info("Output 3.3v: ");
	...
	for (ii = 0; ii < readings.fan_ctrl.fan_count; ii++) {
		...
		msg_info("\tPWM: %i%%\n\tRPM: %i\n", readings.fan_ctrl.speed_pwm, readings.fan_ctrl.rpm_current);
	}
