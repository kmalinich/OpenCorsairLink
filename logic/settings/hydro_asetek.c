...
int
hydro_asetek_settings(struct corsair_device_scan scanned_device, struct option_flags flags, struct option_parse_return settings) {
	int rr;
	int ii;
	char name[20];
	...
	msg_debug("[DBUG] shortcuts set\n");

	rr = dev->driver->init(handle, dev->write_endpoint);
	msg_debug("[DBUG] init done\n");
	...
	for (ii = 0; ii < readings.fan_ctrl.fan_count; ii++) {
		...
		msg_info("\tCurrent/Max Speed %i/%i RPM\n", readings.fan_ctrl.speed, readings.fan_ctrl.rpm_max);
		msg_machine("fan:%d:%d:%i:%i\n", ii, readings.fan_ctrl.mode, readings.fan_ctrl.speed, readings.fan_ctrl.rpm_max);
	}
	...
	msg_info("\tCurrent/Max Speed %i/%i RPM\n", readings.pump_ctrl.speed, readings.pump_ctrl.rpm_max);
	msg_machine("pump:%d:%i:%i\n", readings.pump_ctrl.mode, readings.pump_ctrl.speed, readings.pump_ctrl.rpm_max);
