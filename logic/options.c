...
int
options_parse(int argc, char** argv, struct option_flags* flags, int8_t* device_number, struct option_parse_return* settings) {
...
options_print(void) {
	...
	msg_info("\t\tPWM <PWM percent> : The desired PWM for the selected fan. NOTE: Only works when fan mode is fixed PWM\n");
	msg_info("\t\tRPM <fan RPM>     : The desired RPM for the selected fan. NOTE: Only works when fan mode is fixed RPM\n");
