...
int
corsairlink_unsupported_led(struct corsair_device_info* dev, struct libusb_device_handle* handle, struct led_control* ctrl) {
	msg_info("Unsupported Generic LED Function\n");
	return 0;
}

int
corsairlink_unsupported_fan(struct corsair_device_info* dev, struct libusb_device_handle* handle, struct fan_control* ctrl) {
	msg_info("Unsupported Generic Fan Function\n");
	return 0;
}

int
corsairlink_unsupported_pump(struct corsair_device_info* dev, struct libusb_device_handle* handle, struct pump_control* ctrl) {
...
struct corsair_device_driver corsairlink_driver_asetekpro = {
	...
	.pump = {
		...
		// .profile = corsairlink_asetek_pump_mode,
		.speed = corsairlink_asetekpro_pump_speed_read,
	},
};
